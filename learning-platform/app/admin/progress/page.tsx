...
export default function AdminProgressPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Fetch user list from admin API
  ...
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 4 }}>Student Progress</Typography>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>Modules Active</TableCell>
              <TableCell>AI Tasks (30d)</TableCell>
              <TableCell sx={{ minWidth: 160 }}>Overall Progress</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {students.map((student) => {
              const isOpen = expanded === student.id;
              const avgProgress = student.moduleProgress.length > 0
                ? student.moduleProgress.reduce((s, m) => s + m.percent_complete, 0) / student.moduleProgress.length
                : 0;
              return (
                <React.Fragment key={student.id}>
                  <TableRow hover sx={{ cursor: 'pointer' }} onClick={() => setExpanded(isOpen ? null : student.id)}>
                    ...
                    <TableCell>{student.moduleProgress.length}</TableCell>
                    <TableCell>
                  ...
                  {/* Expanded detail row */}
                  <TableRow>
                    <TableCell colSpan={5} sx={{ p: 0, border: isOpen ? undefined : 0 }}>
                          ...
                          {student.moduleProgress.length === 0 ? (
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, mb: 2 }}>
                              {student.moduleProgress.map((m) => (
                                <Box key={m.module_id} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                  <Typography variant="body2" sx={{ width: 80, fontFamily: 'monospace', flexShrink: 0 }}>
                                    {m.module_id}
                                  </Typography>
                                  <LinearProgress
                                    variant="determinate"
                                  ...
                                  {m.last_module_page_id && (
                                    <Tooltip title={`Last page: ${m.last_module_page_id}`}>
                            ...
                            <>
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                                {student.taskRuns.slice(0, 20).map((r, i) => (
                                  <Tooltip key={i} title={`${r.module_id} · ${new Date(r.created_at).toLocaleDateString()}`}>
